"""
Test BRM Auctions API with correct parameters
"""
import asyncio
import logging
import aiohttp
    ...
    try:
        ...
        test_cases = [
            ...
            },
            {
                ...
                "closeBiddingFrom": (datetime.now() - timedelta(days=7)).strftime('%Y-%m-%dT%H:%M:%S'),
                "closeBiddingTo": (datetime.now() + timedelta(days=1)).strftime('%Y-%m-%dT%H:%M:%S'),
                "description": "Version 2 - With date range (last 7 days to tomorrow)"
            },
            {
                "version": "1",
                "closeBiddingFrom": (datetime.now() - timedelta(days=1)).strftime('%Y-%m-%dT%H:%M:%S'),
                "closeBiddingTo": (datetime.now() + timedelta(hours=24)).strftime('%Y-%m-%dT%H:%M:%S'),
                "description": "Version 1 - Recent auctions (yesterday to tomorrow)"
            }
        ]
